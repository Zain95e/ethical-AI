...
export default function TransparencyDetailPage() {
    ...
    return (
      <Box p={3}>
        <Alert severity="error">
          Failed to load transparency details:{" "}
  ...
  const hasFeatureImportance =
    transparencyData.feature_importance &&
    Object.keys(transparencyData.feature_importance).length > 0;
  const hasModelCard =
    transparencyData.model_card &&
    transparencyData.model_card.performance_metrics;

  if (!hasFeatureImportance && !hasModelCard) {
    return (
      <Box p={3}>
        <Alert severity="warning">
          No transparency data available yet. The validation may still be
          processing or no artifacts were generated.
        </Alert>
        <Button
          startIcon={<ArrowBack />}
          onClick={() => navigate(-1)}
    ...
    <Box sx={{ p: 3, maxWidth: 1400, margin: "0 auto" }}>
      ...
      {transparencyData.warning && (
        <Alert severity="warning" sx={{ mb: 3 }}>
          {transparencyData.warning}. Upload a properly trained model with
          varying predictions and matching feature schema.
        </Alert>
      )}
        ...
        <Card sx={{ mb: 3 }}>
          <CardContent>
            ...
            <Typography variant="body2" color="text.secondary" mb={3}>
              Shows which features have the most influence on the model's
              predictions overall. Higher values indicate greater importance.
            </Typography>
              ...
              <CardContent>
                ...
                <Box>
                  ...
                  <Typography variant="body2" gutterBottom>
                    <strong>Name:</strong>{" "}
                    {transparencyData.model_card.model_details.name}
                  </Typography>
                  <Typography variant="body2" gutterBottom>
                    <strong>Type:</strong>{" "}
                    {transparencyData.model_card.model_details.model_type}
                  </Typography>
                  <Typography variant="body2" gutterBottom>
                    <strong>Features:</strong>{" "}
                    {transparencyData.model_card.model_details.n_features}
                  </Typography>
                  ...
                  {transparencyData.model_card.intended_use && (
                    <Box mt={2}>
                      ...
                      <Typography variant="body2" gutterBottom>
                        ...
                        {transparencyData.model_card.intended_use.primary_use}
                      </Typography>
                  ...
                  <Box mt={2}>
                    <Typography variant="caption" color="text.secondary">
                      MLflow Run:{" "}
                      {transparencyData.mlflow_run_id.substring(0, 8)}...
                    </Typography>
                  </Box>
                </Box>
              </CardContent>
      ...
      {hasFeatureImportance && topFeatures.length > 0 && (
        <Card>
          <CardContent>
              ...
              <Box component="ol" sx={{ m: 0, pl: 2 }}>
                {topFeatures.slice(0, 3).map(([name, value]) => (
                  <li key={name}>
                    <Typography variant="body2">
                      <strong>{name}</strong>:{" "}
                      {(Math.abs(value) * 100).toFixed(2)}% importance
                    </Typography>
                  </li>
                ))}
              </Box>
            ...
            <Typography variant="body2" color="text.secondary">
              💡 <strong>What this means:</strong> The model relies most heavily
              on <strong>{topFeatures[0][0]}</strong> when making predictions.
              Features with higher importance have a greater influence on the
              model's decisions. This transparency helps ensure the model is
              making decisions based on appropriate factors.
            </Typography>
          </CardContent>
        </Card>
      )}
      ...
      {transparencyData.sample_predictions &&
        transparencyData.sample_predictions.length > 0 && (
          <Card sx={{ mt: 3 }}>
            <CardContent>
              ...
              <Typography
                ...
                sx={{ mb: 3 }}
              >
                Below are {transparencyData.sample_predictions.length} example
                predictions showing how specific features contributed to each
                decision. Positive values push toward one class, negative toward
                the other.
              </Typography>
              ...
              <Box display="flex" flexDirection="column" gap={2}>
                {transparencyData.sample_predictions.map((sample, idx) => {
                  ...
                  const maxAbsContribution = Math.max(
                    ...featureEntries.map(([_, data]) =>
                      Math.abs(data.shap_contribution),
                    ),
                  );
                            ...
                            return (
                              <Box key={featureName}>
                                ...
                                <Box display="flex" alignItems="center" gap={1}>
                                  ...
                                  >
                                    {isPositive ? "+" : ""}
                                    {data.shap_contribution.toFixed(3)}
                                  </Typography>
                                </Box>
                              </Box>
                            );
