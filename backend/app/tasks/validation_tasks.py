...
def _get_inner_feature_schema(model_obj: Any):
    """
    Walk an sklearn Pipeline / ColumnTransformer tree to find the step that
    actually declares the input feature schema (feature_names_in_ or n_features_in_).

    Returns (feature_names | None, n_features | None) for the *first* step
    that has schema information.
    """
    # Direct attributes on the object itself
    if hasattr(model_obj, 'feature_names_in_'):
        names = [str(f) for f in model_obj.feature_names_in_]
        return names, len(names)
    if hasattr(model_obj, 'n_features_in_'):
        return None, int(model_obj.n_features_in_)

    # Walk Pipeline steps
    if hasattr(model_obj, 'steps'):
        for _, step in model_obj.steps:
            names, n = _get_inner_feature_schema(step)
            if names is not None or n is not None:
                return names, n

    # Walk ColumnTransformer transformers
    if hasattr(model_obj, 'transformers_'):
        # fitted ColumnTransformer exposes n_features_in_ after fitting
        if hasattr(model_obj, 'n_features_in_'):
            return None, int(model_obj.n_features_in_)
...
def _align_features_to_model(
    ...
    Digs inside sklearn Pipelines and ColumnTransformers to find the real
    input feature schema, then reorders / trims / pads X accordingly so
    that model.predict() does not raise a feature-count mismatch.

    Priority:
    1. Named schema (feature_names_in_) – reorder, drop extras, fill missing.
    2. Count-only schema (n_features_in_) – trim or pad by position.
    3. No schema info – return X unchanged.
    """
    model_obj = model._model if hasattr(model, '_model') else model

    model_features, expected_n = _get_inner_feature_schema(model_obj)
    ...
    if expected_n is not None:
        if X.shape[1] < expected_n:
            if allow_missing_fill:
                missing_count = expected_n - X.shape[1]
                logger.warning(
                    f"{context}: model expects {expected_n} features, got {X.shape[1]}; "
                ...
                raise ValueError(
                    f"{context}: model expects {expected_n} features, "
                    f"but dataset has only {X.shape[1]} after preprocessing"
                )
        if X.shape[1] > expected_n:
            logger.warning(
                f"{context}: model expects {expected_n} features, got {X.shape[1]}; "
                f"dropping {X.shape[1] - expected_n} extra columns by position"
            )
            return X.iloc[:, :expected_n]
