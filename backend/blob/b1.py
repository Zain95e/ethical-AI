...
class AccountabilityTracker:
    ...
    def log_dict(self, data: Dict[str, Any], filename: str) -> None:
        ...
        try:
            ...
            # Save the artifact via MLflow
            self._mlflow.log_dict(data, filename)

            # Verify the file was actually saved using cross-platform pathlib
            artifact_uri = self._current_run.info.artifact_uri
            local_path = self._artifact_uri_to_path(artifact_uri)
            expected_path = local_path / filename

            if expected_path.exists():
                logger.info(f"✅ SUCCESS: Saved {filename} ({expected_path.stat().st_size} bytes) to {expected_path}")
            else:
                logger.warning(f"⚠️ File saved but not found at expected path: {expected_path}")
                if local_path.exists():
                    files = list(local_path.iterdir())
                    logger.info(f"📁 Files in artifact dir: {[f.name for f in files]}")
                else:
                    logger.warning(f"Artifact directory does not exist yet: {local_path} (MLflow may buffer writes)")
    ...
    def _artifact_uri_to_path(artifact_uri: str) -> Path:
        ...
        Handles all formats on both Windows and Linux:
          - file:///absolute/path  (Linux)
          - file:///C:/absolute/path  (Windows)
          - file://localhost/path
          - /absolute/path  (bare POSIX path)
          - ./relative/path
        """
        ...
        if uri.startswith("file://"):
            parsed = urlparse(uri)
            # unquote handles %20 etc.; Path normalises separators per-OS
            path_str = unquote(parsed.path)
            # On Windows urlparse keeps a leading /C:/... — pathlib handles it
